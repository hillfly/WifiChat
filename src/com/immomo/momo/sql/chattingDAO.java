...
public class chattingDAO
{
	...
	/*参数：发送方ID sendID,接收方ID receiverID
	 *结果返回一系列的聊天记录的ID
	 */
	public List<Integer> getID(int sendID,int receiverID)
	{
		...
		Cursor cursor=db.query(helper.getTableName(), new String[]{"id"}, 
				"sendID=? and receiverID=?", new String[]{ String.valueOf(sendID),String.valueOf(receiverID) }, null, null, null);
	...
	public List<chattingInfo> getAllMessage(int sendID,int receiverID)
	{
		...
		Cursor cursor=db.query(helper.getTableName(), new String[]{"id","sendID","receiverID","chatting","date"}, 
				"sendID=? and receiverID=?", new String[]{ String.valueOf(sendID),String.valueOf(receiverID) }, null, null, null);
		while (cursor.moveToNext())
		{
			infos.add(new chattingInfo(cursor.getInt(cursor.getColumnIndex("id")),
...
//		db=helper.getWritableDatabase();
		...
		values.put("chatting", info.getInfo());
		values.put("date", info.getDate());
		db.update(helper.getTableName(), values, "id = ?", new String[]{ String.valueOf( info.getId() ) } );
	}
	/* 参数：聊天记录序号ID
	 * 作用:用来查找对应的一条聊天记录
	 * 返回chattinginfo类
...
//		db = helper.getWritableDatabase();
		Cursor cursor = db.query(helper.getTableName(), new String[]
		{ "id","sendID","receiverID","chatting","date"}, null, null, null, null, "id desc", start + "," + count);
		while (cursor.moveToNext())
		{
			info.add(new chattingInfo(cursor.getInt(cursor.getColumnIndex("id")),
					cursor.getInt(cursor.getColumnIndex("sendID")),
					cursor.getInt(cursor.getColumnIndex("receiverID")),
					cursor.getString(cursor.getColumnIndex("chatting")),
					cursor.getString(cursor.getColumnIndex("date"))));
